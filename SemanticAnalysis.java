...
public class SemanticAnalysis implements Visitor {
  ...
  public void check(Program progAST) {
    ...
    /* Start of your code: */
    Decl mainD = scopeStack.retrieve("main");
    if (mainD == null) {
  ...
  public void visit(FunDecl x) {
    ...
    /* Start of your code: */
    if (!scopeStack.enter(x.idAST.Lexeme, x)) { //  but if fails,
    ...
    if (x.idAST.Lexeme.equals("main")) {
    	if (!currentFunctionReturnType.Tequal(StdEnvironment.intType)) // if the main function's return value isn't int
  ...
  public void visit(FormalParamDecl x) {
    ...
    /* Start of your code: */
    if (!scopeStack.enter(x.astIdent.Lexeme, x)) { // try to enter. but if fails,
    ...
    /* Start of your code: */
    Type T = typeOfDecl(x);
    if (T.Tequal(StdEnvironment.voidType)) {
    	reporter.reportError(errMsg[3], "", x.astType.pos);
    }
    else if (T instanceof ArrayType && ((ArrayType)T).astType.Tequal(StdEnvironment.voidType)) {
  ...
  public void visit(AssignStmt x) {
    ...
    if (!x.rAST.type.AssignableTo(x.lAST.type)) {
      reporter.reportError(errMsg[6], "", x.rAST.pos);
    	...
    	// 2) case of 'float = int', we have to i2f
  ...
  public void visit(ReturnStmt x) {
    if (!(x.eAST instanceof EmptyExpr)) {
      ...
      /* START: */
      if(x.eAST.type.AssignableTo(currentFunctionReturnType)) { // if the eAST's type is assignable to the current func return type.
  ...
  public void visit(CompoundStmt x) {
    ...
    /* Start of your code: */
    x.astDecl.accept(this); // decl-seq
    x.astStmt.accept(this); // stmt-seq
    /* End of your code!! */
...
//    scopeStack.closeScope();
  ...
  public void visit(VarDecl x) {
    ...
    if (!(x.eAST instanceof EmptyExpr)) {
      ...
      if (x.tAST instanceof ArrayType) {
        ...
        /* Start of your code: */
      	...
      	// 15. scalar initializer (ex. a[3] = 1)
      	if (!(x.eAST instanceof ExprSequence)) {
      		reporter.reportError(errMsg[15], "", x.pos);
      	}
      	else { // vector initializer
      		// 16. too many elements (ex. a[3] = {1, 2, 3, 4})
        	int num = ((ArrayType)x.tAST).GetRange();
        	Expr e = x.eAST;
        	for (int loop=0; loop<num ; loop++) {
        		if (!(e instanceof EmptyExpr)) e = ((ExprSequence)e).rAST;
        	}
        	// if the elements <= num, e should be EmptyExpr.
        	if (!(e instanceof EmptyExpr)) { 
        		reporter.reportError(errMsg[16], "", x.pos);
        	}
        	else { // not too many
        		// 13. wrong type element (ex. int a[2] = {2, false})
          	// float a[2] = {1, 2.0} is allowed. i2f needed! f = i;
          	Expr cur = x.eAST;
          	while (!(cur instanceof EmptyExpr)) {
          		Type elementType = ((ExprSequence)cur).lAST.type;
          		Type decType = ((ArrayType)x.tAST).astType;
          		if(elementType.AssignableTo(decType)) {
    	          if(decType.Tequal(StdEnvironment.floatType) &&
    	          		elementType.Tequal(StdEnvironment.intType)) {
    	            //coercion of operand to float:
    	          	((ExprSequence)cur).lAST = i2f(((ExprSequence)cur).lAST);
    	          }
              }
              else {
              	reporter.reportError(errMsg[13], "", ((ExprSequence)cur).lAST.pos);
              	return;
              }
          		cur = ((ExprSequence)cur).rAST;
          	}
        	}
      	}
        ...
        /* Start of your code: */
          ...
          // case of 'float = int', we have to i2f
    ...
    /* Start of your code: */
    if (!scopeStack.enter(x.idAST.Lexeme, x)) { // try to enter. but if fails,
    	reporter.reportError(errMsg[2], "", x.idAST.pos);
    }
    /* End of your code!! */
  ...
  public void visit(VarExpr x) {
    ...
    /* Start of your code: */
    // int a = c <- varExpr = c
    // this expr's decl should be var,
    // but if it if function decl, error!
